...
time aws s3 cp \
  ...
  s3://$S3_BUCKET_PATH/$DATABASE_NAME/"${filename}"_plain_format.gz.enc

printf "upload custom format ...\n"
time aws s3 cp \
  /tmp/"${filename}"_custom_format.enc \
  s3://$S3_BUCKET_PATH/$DATABASE_NAME/"${filename}"_custom_format.enc

printf "upload directroy format ...\n"
time aws s3 cp \
  /tmp/"${filename}"_directory_format.gz.enc \
  s3://$S3_BUCKET_PATH/$DATABASE_NAME/"${filename}"_directory_format.gz.enc

printf "upload tar format ...\n"
time aws s3 cp \
  /tmp/"${filename}"_tar_format.gz.enc \
  s3://$S3_BUCKET_PATH/$DATABASE_NAME/"${filename}"_tar_format.gz.enc
